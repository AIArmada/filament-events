...
- Role: Filament admin UI for events, occurrences, sessions, venues, registrations, attendance, check-in, and moderation workflows.
- Search first: `src/Resources`, `src/Pages`, `src/Widgets`, `src/RelationManagers`, `src/Actions`, `src/Support`, `config`, `docs`
- Related: `events`, `ticketing`, `filament-ticketing`, `commerce-support`
...
- Keep ticket type and pass administration in `filament-ticketing`; only surface event-specific ticketing views here when the workflow is inherently event-scoped.
